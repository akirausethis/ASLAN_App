// JapaneseFlashCardListView.swift
import SwiftUI

struct JapaneseFlashCardListView: View {
    @State private var selectedLevel: FlashcardLevel = .beginner
    // Ubah selectedCourseId menjadi activeCourseId untuk kontrol navigasi
    @State private var activeCourseId: UUID? = nil

    // Our sample courses for the Beginner level
    let beginnerCourses: [JapaneseCourse] = [
        JapaneseCourse(title: "Hiragana", subtitle: "Course Title", iconName: "text.book.closed.fill"),
        JapaneseCourse(title: "Katakana", subtitle: "Course Title", iconName: "text.book.closed.fill")
    ]

    var body: some View {
        NavigationView {
            VStack(alignment: .leading) {
                // Header: Back button and Title
                HStack {
                    Spacer()
                    Text("Flashcard")
                        .font(.largeTitle)
                        .fontWeight(.bold)
                        .foregroundColor(.blue)
                    Spacer()
                }
                .padding(.horizontal)
                .padding(.top, 10)

                // Level Selection Buttons
                HStack(spacing: 10) {
                    Spacer()
                    ForEach(FlashcardLevel.allCases) { level in
                        Button(action: {
                            selectedLevel = level
                            // Reset active course when level changes
                            activeCourseId = nil
                        }) {
                            Text(level.rawValue)
                                .font(.subheadline)
                                .fontWeight(.medium)
                                .padding(.vertical, 8)
                                .padding(.horizontal, 15)
                                .background(selectedLevel == level ? Color.blue : Color.white)
                                .foregroundColor(selectedLevel == level ? .white : .blue)
                                .cornerRadius(10)
                                .overlay(
                                    RoundedRectangle(cornerRadius: 10)
                                        .stroke(Color.blue, lineWidth: 1)
                                )
                        }
                    }
                    Spacer()
                }
                .padding(.horizontal)
                .padding(.vertical, 5)


                // Flashcard Course List (Only for Beginner for now)
                ScrollView {
                    VStack(spacing: 15) {
                        if selectedLevel == .beginner {
                            ForEach(beginnerCourses) { course in
                                // Menggunakan binding activeCourseId untuk NavigationLink
                                NavigationLink(
                                    destination:
                                        Group {
                                            if course.title == "Hiragana" {
                                                HiraganaFlashcardCarouselView()
                                            } else if course.title == "Katakana" {
                                                KatakanaFlashcardCarouselView()
                                            }
                                            // Handle other courses if needed, or leave empty if not navigatable
                                        },
                                    isActive: Binding(
                                        get: { self.activeCourseId == course.id },
                                        set: { isActive in
                                            if isActive {
                                                self.activeCourseId = course.id
                                            } else if self.activeCourseId == course.id {
                                                // Ini akan diatur nil ketika NavigationLink di-dismiss
                                                self.activeCourseId = nil
                                            }
                                        }
                                    )
                                ) {
                                    // Label untuk NavigationLink, yaitu JapaneseFlashCardView
                                    JapaneseFlashCardView(course: course, isSelected: activeCourseId == course.id)
                                }
                                // .simultaneousGesture tidak diperlukan lagi di sini karena activeCourseId mengelola state
                            }
                        } else {
                            Text("No courses available for \(selectedLevel.rawValue) yet.")
                                .foregroundColor(.gray)
                                .padding()
                        }
                    }
                    .padding(.bottom, 20)
                }
                .frame(maxWidth: .infinity, maxHeight: .infinity)

                Spacer()
            }
            .navigationBarHidden(true)
        }
    }
}

#Preview {
    JapaneseFlashCardListView()
}
