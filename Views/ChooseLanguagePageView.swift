...
struct ChooseLanguagePageView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 0) {
                ...
                VStack(spacing: 15) {
                    ForEach(languages, id: \.self) { language in
                        Button(action: {
                            userViewModel.chosenLanguage = language
                        }) {
                            HStack {
                                ...
                                Text(language).font(.headline)
                                    .foregroundColor(userViewModel.chosenLanguage == language ? .white : .black)
                                Spacer()
                                if userViewModel.chosenLanguage == language {
                                    Circle().frame(width: 20, height: 20)
                                        .foregroundColor(activeSelectionColor)
                                }
                            }
                            .padding().frame(maxWidth: .infinity)
                            .background(userViewModel.chosenLanguage == language ? activeSelectionColor : Color.white)
                ...
                Button(action: {
                    if userViewModel.chosenLanguage != nil {
                        print("Continue button pressed, selected language: \(userViewModel.chosenLanguage!)")
                    ...
                    Text("Continue")
                        ...
                        // 6. Gunakan UserViewModel untuk kondisi UI
                        .background(userViewModel.chosenLanguage != nil ? activeSelectionColor : Color.gray.opacity(0.5))
                        .cornerRadius(10)
                }
                .disabled(userViewModel.chosenLanguage == nil)
            ...
            .background(
                NavigationLink(
                    destination: ChooseLevelPageView(userViewModel: self.userViewModel)                        .navigationBarBackButtonHidden(true),
