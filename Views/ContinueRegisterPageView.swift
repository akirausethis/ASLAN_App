...
struct ContinueRegisterPageView: View {
    ...
    private func markAsContinuedFromRegister() {
        guard let currentUser = Auth.auth().currentUser else {
            print("Error: User not authenticated yet for ContinueRegisterPageView status.")
        ...
        let statusData: [String: Any] = [
            "hasSeenContinueRegisterPage": true,
        ...
        dbRef.child("users").child(userID).child("onboardingStatus").child("passedContinueRegister").setValue(statusData) { error, _ in
            DispatchQueue.main.async {
                ...
                if let error = error {
                    print("Error saving 'passedContinueRegister' status: \(error.localizedDescription)")
                    self.saveError = "Could not save status: \(error.localizedDescription)"
                } else {
                    print("'passedContinueRegister' status saved successfully.")
                }
                self.navigateToChooseLanguage = true
            }
        }
